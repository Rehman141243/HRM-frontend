...
import { cn } from "@/lib/utils";
import { getUser } from "@/lib/auth";
import { DesignationBadge, EmptyState, fmtNum, fmtPKR, getPermissions, MONTH_NAMES, MONTH_SHORT, normalizePayroll, normalizeSalaryStructure, StatusBadge, extractErrorMessage, buildPayslipHtml } from "../../../../components/modal-components/modalcomponents";
import PoliciesTab from "../../../../components/modal-components/policy-tab";
import EmployeePayrollHistoryModal from "../../../../components/modals/EmployeePayrollHistoryModal";
import SalaryStructureModal from "../../../../components/modals/SalaryStructureModal";
import EditStructureModal from "../../../../components/modals/EditStructureModal";
...
export default function PayrollService() {
  ...
  const loadStructures = useCallback(async (p) => {
    ...
    try {
      ...
      if (perms.canViewAllStructures) {
        const res = await axiosInstance.get("/salary-structures", { params: { page: p, limit: 10 } });
        rawStructures = res.data.salary_structures ?? [];
        setStructPagination(res.data.pagination ?? { total: 0, pages: 1 });
      } else {
        const empId = perms.selfEmployeeId;
        if (!empId) { toast.error("Cannot determine your employee ID."); setStructures([]); return; }
        const res = await axiosInstance.get(`/salary-structures/employee/${empId}`);
        const s = res.data?.salary_structure || res.data;
        rawStructures = s?.id ? [s] : [];
        setStructPagination({ total: rawStructures.length, pages: 1 });
      }

      // Fetch all policy types in parallel and build an id→policy map
      const [att, ovt, tax, bon] = await Promise.allSettled([
      ...
      const policyMap = {};
      [att, ovt, tax, bon].forEach(r => {
        if (r.status === "fulfilled") {
          (r.value.data.policies || []).forEach(pol => { policyMap[pol.id] = pol; });
        }
      });

      // Attach policy objects to each structure
      const enriched = rawStructures.map(s => ({
        ...s,
        attendance_policy: policyMap[s.attendance_policy_id] || s.attendance_policy || null,
        overtime_policy:   policyMap[s.overtime_policy_id]   || s.overtime_policy   || null,
        tax_policy:        policyMap[s.tax_policy_id]         || s.tax_policy         || null,
        bonus_policy:      policyMap[s.bonus_policy_id]       || s.bonus_policy       || null,
      }));
