...
export const attendanceColumns = ({ onRaiseRegularization }) => [
	{
		...
		cell: ({ row }) => (
			<div className="space-y-1">
				<p className="text-sm font-medium leading-none">{row.original.attendance_id || row.original.id || "--"}</p>
	...
	},
	{
		accessorKey: "date",
		header: "Date",
		cell: ({ row }) => <span className="text-sm font-medium">{fmtDate(row.original.date || row.original.attendance_date)}</span>,
	},
	{
		accessorKey: "status",
		header: "Status",
		cell: ({ row }) => {
			const meta = getAttendanceStatusMeta(row.original);
			return (
	...
	},
	{
		...
		header: "Check In",
		cell: ({ row }) => <span className="text-sm text-muted-foreground">{formatTime(row.original.check_in_time_local || row.original.check_in_time)}</span>,
	},
	{
		accessorKey: "check_out_time",
		header: "Check Out",
		cell: ({ row }) => <span className="text-sm text-muted-foreground">{formatTime(row.original.check_out_time_local || row.original.check_out_time)}</span>,
	},
	{
		id: "actions",
		header: "",
		cell: ({ row }) => {
			...
			return (
				<div className="flex justify-end">
					<Button type="button" size="sm" className="gap-2" disabled={!attendanceId} onClick={() => onRaiseRegularization?.(row.original)}>
...
export const regularizationColumns = ({ onViewRequest }) => [
	{
		accessorKey: "attendance_id",
		header: "Attendance ID",
		cell: ({ row }) => <span className="text-sm font-medium">{row.original.attendance_id || "--"}</span>,
	},
	{
		accessorKey: "type",
		header: "Type",
		cell: ({ row }) => {
			...
			return (
				<div className="space-y-1">
					...
					{customType && <p className="text-xs text-muted-foreground">Custom: {customType}</p>}
				</div>
			);
		},
	},
	{
		accessorKey: "reason",
		header: "Reason",
		cell: ({ row }) => <span className="line-clamp-2 text-sm text-muted-foreground">{row.original.reason || "--"}</span>,
	},
	{
		accessorKey: "status",
		header: "Status",
		cell: ({ row }) => {
			const meta = getRequestStatusMeta(row.original.status);
			return (
				<Badge variant="outline" className={`text-xs font-medium ${meta.className}`}>
					{meta.label}
				</Badge>
	...
	},
	{
		...
		header: "Submitted",
		cell: ({ row }) => <span className="text-sm text-muted-foreground">{fmtDate(row.original.submitted_at || row.original.created_at)}</span>,
	},
	{
		id: "actions",
		header: "",
		cell: ({ row }) => (
			<div className="flex justify-end">
				<Button type="button" variant="outline" size="sm" className="gap-2" onClick={() => onViewRequest?.(row.original)}>
