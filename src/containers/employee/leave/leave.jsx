...
//           <p className="text-xs sm:text-sm text-muted-foreground">Manage and track your leave applications</p>
//         </div>
//         <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
//           <DialogTrigger asChild>
//             <Button size="sm" className="gap-1.5 text-xs sm:text-sm"><CalendarPlus className="h-4 w-4" />Apply Leave</Button>
//           </DialogTrigger>
...
//                   }}
//                 >
//                   <SelectTrigger className="h-8 w-35 text-xs sm:text-sm">
...
//                     avoidCollisions={false}
//                   >
//                     <SelectItem value="desc" className="text-xs sm:text-sm">Newest First</SelectItem>
//                     <SelectItem value="asc" className="text-xs sm:text-sm">Oldest First</SelectItem>
//                   </SelectContent>
//                 </Select>
...
const leaveCardFields = [
  {
    label: "Leave Type",
    accessor: (row) => leaveTypeLabel[row.leave_type] || row.leave_type || "--",
  },
  {
    label: "From",
    accessor: (row) => fmtDate(row.start_date || row.leave_date) || "--",
    className: "text-muted-foreground",
  },
  {
    label: "To",
    accessor: (row) =>
      row.leave_type === "full_day"
        ? fmtDate(row.end_date) || "--"
        : row.leave_type === "short_leave"
        ? `${row.start_time || "--"} – ${row.end_time || "--"}`
        : row.half_day_type
        ? String(row.half_day_type).replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())
        : "--",
    className: "text-muted-foreground",
  },
  {
    label: "Applied On",
    accessor: (row) => fmtDate(row.created_at || row.applied_at) || "--",
    className: "text-muted-foreground",
  },
  {
    label: "Reason",
...
export default function Leave() {
  ...
  useEffect(() => {
    fetchLeaves(0, pageSize);
  }, [fetchLeaves, pageSize]);
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-1.5">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="full_day" className='text-xs sm:text-sm'>Full Day</SelectItem>
                    <SelectItem value="half_day" className='text-xs sm:text-sm'>Half Day</SelectItem>
                    <SelectItem value="short_leave" className='text-xs sm:text-sm'>Short Leave</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div
                className={`grid gap-3 ${form.leave_type === "full_day" ? "grid-cols-2" : "grid-cols-1"}`}
              >
                <div className="space-y-1.5">
                  <Label>
                    {form.leave_type === "full_day" ? "From Date" : "Date"}
                  </Label>
                  <Input
                    type="date"
              ...
              {form.leave_type === "half_day" && (
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setForm({ ...form, half_day_type: v })}
                  >
                    <SelectTrigger  className='text-xs sm:text-sm '>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="morning"  className='text-xs sm:text-sm '>Morning</SelectItem>
                      <SelectItem value="evening"  className='text-xs sm:text-sm '>Evening</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              )}
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="block md:hidden">
                <MobileCardList
                  data={filteredLeaves}
                  fields={leaveCardFields}
                  highlight={leaveCardHighlight}
                  actions={(row) => {
