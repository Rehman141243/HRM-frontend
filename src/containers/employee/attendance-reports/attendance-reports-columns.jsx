...
export const attendanceReportColumns = [
	{
		accessorKey: "date",
		header: "Date",
		cell: ({ row }) => <span className="text-sm font-medium">{fmtDate(row.original.date)}</span>,
	},
	{
		...
		cell: ({ row }) => (
			<div className="space-y-1">
				...
				<div className="text-xs text-muted-foreground">
					{row.original.shift?.start_time && row.original.shift?.end_time
	...
	},
	{
		...
		cell: ({ row }) => (
			<span className="text-sm text-muted-foreground">
				{row.original.check_in_time_local ? formatTime(row.original.check_in_time_local.split("T")[1]) : "—"}
			</span>
		),
	},
	{
		...
		cell: ({ row }) => (
			<span className="text-sm text-muted-foreground">
				{row.original.check_out_time_local ? formatTime(row.original.check_out_time_local.split("T")[1]) : "—"}
			</span>
		),
	},
	{
		...
		cell: ({ row }) => (
			<span className="text-sm text-muted-foreground">
				{row.original.worked_hours != null ? `${row.original.worked_hours}h` : "—"} / {row.original.shift_hours ?? "—"}h
			</span>
		),
	},
	{
	...
	},
	{
		accessorKey: "late",
		header: "Attendance",
		cell: ({ row }) => getLateIndicator(row.original.evaluation),
	},
];
