...
import TableToolbar from "@/components/common/table-toolbar";
import { MONTH_NAMES, MONTH_SHORT, StatusBadge, fmtNum, fmtPKR, normalizePayroll } from "@/components/modal-components/modalcomponents";
import { CreditCard, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import { getPayrollColumns } from "./payroll-columns";
import { useRouter } from "next/navigation";
...
const payrollCardFields = [
    // ── Header title: Period ──────────────────────────────────────────────────
    {
        label: "Period",
        accessor: (row) => {
            const month = row?.period?.month || 1;
            const year  = row?.period?.year  || "";
            return `${MONTH_SHORT[(month - 1)] ?? ""} ${year}`.trim() || "—";
        },
    ...
    // ── 2-col grid ────────────────────────────────────────────────────────────
    {
        ...
        accessor: (row) => (
            <span className="text-sm tabular-nums text-muted-foreground">
                {fmtNum(row?.period?.working_days)}
            </span>
        ),
    },
    {
        ...
        accessor: (row) => (
            <span className="text-sm tabular-nums font-bold text-primary">
                {fmtNum(row?.attendance?.payable_days)}
            </span>
        ),
    },

    // ── Full-width: Gross Salary ──────────────────────────────────────────────
    {
        label: "Gross",
        accessor: (row) => (
            <span className="text-sm font-mono tabular-nums text-muted-foreground">
                {fmtPKR(row?.totals?.gross_salary)}
            </span>
        ),
        fullWidth: true,
    },
    ...
    // ── Full-width: Deductions ────────────────────────────────────────────────
    {
        ...
        accessor: (row) => (
            <span className="text-sm font-mono tabular-nums text-red-600 dark:text-red-400">
                -{fmtPKR(row?.totals?.deductions_total)}
            </span>
        ),
        fullWidth: true,
    },

    // ── Full-width: Net Salary ────────────────────────────────────────────────
    {
        label: "Net Salary",
        accessor: (row) => (
            <span className="text-sm font-mono tabular-nums font-bold">
                {fmtPKR(row?.totals?.net_salary)}
            </span>
        ),
        fullWidth: true,
    },
...
export default function Payroll() {
    ...
    const employeeUserId = useMemo(() => user?.employee_id ?? null, [user]);
    ...
    const handlePageChange = (nextPage) => {
        setPageIndex(nextPage);
        if (!searchTerm.trim()) loadMyPayrolls(nextPage + 1, pageSize);
    };

    const handlePageSizeChange = (nextSize) => {
        setPageIndex(0);
        setPageSize(nextSize);
        if (!searchTerm.trim()) loadMyPayrolls(1, nextSize);
    };
            ...
            <Card className="border-border/60 shadow-sm">
                <CardContent className="pt-4 space-y-4">
                    ...
                    <div className="block md:hidden">
                        <MobileCardList
                            data={filteredPayrolls}
                            fields={payrollCardFields}
                            highlight={payrollCardHighlight}
                            actions={(row) => (
                                <Button
                                    size="sm"
                                    variant="ghost"
                                    className="w-full h-7 px-2 text-xs text-primary hover:bg-primary/10 hover:text-primary"
