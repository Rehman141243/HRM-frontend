  ...
  const loadStructures = useCallback(async (p) => {
    ...
    try {
      if (perms.canViewAllStructures) {
        ...
        // Resolve employee UUID dynamically if not in user object
        let empId = perms.selfEmployeeId;
        if (!empId) {
          const meRes = await axiosInstance.get("/payroll/me", { params: { page: 1, limit: 1 } });
          const payroll = meRes.data?.payrolls?.[0] || meRes.data?.payroll;
          empId = payroll?.employee_id ?? null;
        }
        if (!empId) {
