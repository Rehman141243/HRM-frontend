...
export default function ManagerOvertimeTab() {
  ...
  // GET /attendance/overtime-requests?status=pending
  ...
  // PUT /attendance/overtime-requests/:id/approve or /reject
  ...
  return (
    <div className="space-y-4 mt-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-semibold">Overtime Approvals</h3>
          <p className="text-sm text-muted-foreground">Review and act on overtime requests</p>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>Employee</TableHead><TableHead>Date</TableHead><TableHead>Time</TableHead>
                  <TableHead>Hours</TableHead><TableHead>Reason</TableHead><TableHead>Status</TableHead><TableHead>Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {requests.length === 0 ? (
                  <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8 text-sm">No pending overtime requests.</TableCell></TableRow>
                ) : (
                  requests.map((r) => (
                    <TableRow key={r.id}>
