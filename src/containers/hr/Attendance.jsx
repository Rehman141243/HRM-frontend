...
import { calcDuration, fmtTime, StatusBadge,fmtDate  } from "@/components/common/common";
...
{status.check_in_time && (
  <div className="text-xs text-muted-foreground">
    In: <span className="font-medium text-foreground">
      {fmtTime(status.check_in_time_local || status.check_in_time)}
    </span>
    {status.check_out_time && (
      <> · Out: <span className="font-medium text-foreground">
        {fmtTime(status.check_out_time_local || status.check_out_time)}
      </span></>
    )}
  </div>
)}
