...
import { buildBonusPolicyColumns } from "./bonus-policy-columns";
import { Plus, RefreshCw, Shield } from "lucide-react";
import { API_PATH, POLICY_META, extractErrorMessage } from "./bonus-policy-utils";

export function BonusPolicyList({
  showHeader = true,
  basePath = "/hr",
}) {
  ...
  const goToCreate = () => router.push(`${basePath}/policies-structure/policies/create?type=bonus`);
  const goToEdit = (policy) => router.push(`${basePath}/policies-structure/policies/${policy.id}?type=bonus`);

  const columns = useMemo(() => buildBonusPolicyColumns({
        ...
        <div className="p-4">
          {loading ? (
            <div className="flex items-center justify-center gap-2 py-20 text-gray-400">
              ...
              <span className="text-sm">Loading bonus policies…</span>
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center px-4">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {search
                  ...
                  : `No bonus policies yet.${perms.canManagePolicies ? " Create one to get started." : ""}`}
              </p>
            </div>
          ) : (
            <DataTable
...
export default BonusPolicyList;
