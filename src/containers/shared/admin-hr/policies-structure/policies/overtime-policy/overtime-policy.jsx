...
import { buildOvertimePolicyColumns } from "./overtime-policy-columns";
import { Plus, RefreshCw, Shield } from "lucide-react";
import { API_PATH, POLICY_META, extractErrorMessage } from "./overtime-policy-utils";

export function OvertimePolicyList({
  showHeader = true,
  basePath = "/hr",
}) {
  ...
  const goToCreate = () => router.push(`${basePath}/policies-structure/policies/create?type=overtime`);
  const goToEdit = (policy) => router.push(`${basePath}/policies-structure/policies/${policy.id}?type=overtime`);

  const columns = useMemo(() => buildOvertimePolicyColumns({
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 overflow-hidden">
        <div className="p-4 border-gray-200 dark:border-gray-800">
          <TableToolbar
            placeholder="Search overtime policies…"
        ...
        <div className="p-4">
          {loading ? (
            <div className="flex items-center justify-center gap-2 py-20 text-gray-400">
              ...
              <span className="text-sm">Loading overtime policies…</span>
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center px-4">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {search
                  ...
                  : `No overtime policies yet.${perms.canManagePolicies ? " Create one to get started." : ""}`}
              </p>
            </div>
          ) : (
            <DataTable
...
export default OvertimePolicyList;
