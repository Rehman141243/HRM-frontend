...
export default function PolicyModal({ open, onClose, type, policy, onSave, loading }) {
  ...
  const handleSubmit = () => {
    ...
    if (type === "attendance") {
      // Numeric fields
      ["grace_minutes_default", "late_count_for_unpaid_day", "min_hours_for_present", "min_hours_for_half_day", "full_day_hours", "short_hours_payable"].forEach((k) => {
        if (payload[k] !== "") payload[k] = Number(payload[k]);
      });

      // String enum fields — keep as-is (no conversion needed)
      // no_checkout_behavior, short_hours_behavior are already strings from select

      // Date array fields — remove if empty, otherwise trim
      const dateFields = ["holiday_dates", "working_weekend_dates", "forced_working_dates", "manual_off_dates"];
      dateFields.forEach((field) => {
        if (payload[field] === "") {
  ...
  // Shared select class (reuse inputCls but keep selects consistent)
  const selectCls = cn(inputCls, "dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100");
  // Optional badge
  const Opt = () => <span className="ml-1 text-[10px] font-normal text-gray-400 dark:text-gray-500">(optional)</span>;
    ...
    <Modal open={open} onClose={onClose} maxWidth="max-w-lg">
      ...
      <ModalBody>
        <div className="px-6 py-5 space-y-4">
          ...
          {type === "attendance" && (
            <>
              {/* Row 1: Timezone + Grace Minutes */}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>Timezone<Opt /></label>
                  <input className={inputCls} placeholder="Asia/Karachi" value={form.timezone || ""} onChange={setF("timezone")} />
                </div>
                <div>
                  <label className={labelCls}>Grace Minutes<Opt /></label>
                  <input type="number" min="0" className={inputCls} value={form.grace_minutes_default ?? ""} onChange={setN("grace_minutes_default")} />
                </div>
              </div>

              {/* Row 2: Lates → Unpaid Day + Weekly Off Days */}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>Lates → Unpaid Day<Opt /></label>
                  <input type="number" min="1" className={inputCls} value={form.late_count_for_unpaid_day ?? ""} onChange={setN("late_count_for_unpaid_day")} />
                </div>
                <div>
                  <label className={labelCls}>Weekly Off Days<Opt /></label>
                  <input className={inputCls} placeholder="0,6 (Sun,Sat)" value={form.weekly_off_days || ""} onChange={setF("weekly_off_days")} />
                </div>
              </div>

              {/* ── Hour Thresholds ── */}
              <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest pt-1">Hour Thresholds</p>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className={labelCls}>Min Hours Present</label>
                  <input type="number" min="0" step="0.5" className={inputCls} value={form.min_hours_for_present ?? ""} onChange={setN("min_hours_for_present")} />
                </div>
                <div>
                  <label className={labelCls}>Min Hours Half Day</label>
                ...
                <div>
                  <label className={labelCls}>Full Day Hours</label>
                  <input type="number" min="0" step="0.5" className={inputCls} value={form.full_day_hours ?? ""} onChange={setN("full_day_hours")} />
                </div>
              </div>

              {/* ── Behavior Rules ── */}
              <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest pt-1">Behavior Rules</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>No Checkout Behavior</label>
                ...
                <div>
                  <label className={labelCls}>Short Hours Behavior</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>Short Hours Payable Fraction</label>
                  <select className={selectCls} value={String(form.short_hours_payable ?? 1)} onChange={(e) => setForm((f) => ({ ...f, short_hours_payable: Number(e.target.value) }))}>
              ...
              {/* ── Date Overrides ── */}
              <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest pt-1">Date Overrides</p>
              <div>
                <label className={labelCls}>Holiday Dates<Opt /></label>
                <input className={inputCls} placeholder="2025-12-25,2026-01-01" value={form.holiday_dates || ""} onChange={setF("holiday_dates")} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>Working Weekend Dates<Opt /></label>
                  <input className={inputCls} placeholder="2025-12-27" value={form.working_weekend_dates || ""} onChange={setF("working_weekend_dates")} />
                </div>
                <div>
                  <label className={labelCls}>Manual Off Dates<Opt /></label>
                  <input className={inputCls} placeholder="2025-12-26" value={form.manual_off_dates || ""} onChange={setF("manual_off_dates")} />
                </div>
              </div>
              <div>
                <label className={labelCls}>Forced Working Dates<Opt /></label>
                <input className={inputCls} placeholder="2025-12-30" value={form.forced_working_dates || ""} onChange={setF("forced_working_dates")} />
              </div>

              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.apply_proration_default ?? true} onChange={setF("apply_proration_default")} className="w-4 h-4 rounded accent-gray-900 dark:accent-gray-300" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Apply proration by default</span>
              </label>
            </>
          )}
          ...
          {type === "overtime" && (
            <>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className={labelCls}>Min OT/Day (h)</label>
                  <input type="number" min="0" step="0.5" className={inputCls} value={form.min_hours_per_day ?? ""} onChange={setN("min_hours_per_day")} />
                </div>
                <div>
                  <label className={labelCls}>Max OT/Day (h)</label>
                  <input type="number" min="0" step="0.5" className={inputCls} value={form.max_hours_per_day ?? ""} onChange={setN("max_hours_per_day")} />
                </div>
                <div>
                  <label className={labelCls}>Max OT/Month (h)</label>
                  <input type="number" min="0" className={inputCls} value={form.max_hours_per_month ?? ""} onChange={setN("max_hours_per_month")} />
                </div>
              </div>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.apply_proration_default ?? false} onChange={setF("apply_proration_default")} className="w-4 h-4 rounded accent-gray-900 dark:accent-gray-300" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Apply proration to overtime</span>
              </label>
            </>
          )}
              ...
              {form.tax_mode_default === "slab" && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={addSlab} className="inline-flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium">
                      <Plus className="w-3 h-3" />Add Slab
                    </button>
                  </div>
                  <div className="space-y-2">
                    {(form.tax_slabs || []).map((slab, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <button onClick={() => removeSlab(i)} className="text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                  </div>
                  <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Leave "Up to" empty for the top slab (∞)</p>
                </div>
              )}
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={form.apply_proration_default ?? false} onChange={setF("apply_proration_default")} className="w-4 h-4 rounded accent-gray-900 dark:accent-gray-300" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Apply proration to fixed tax</span>
              </label>
            </>
          )}
          ...
          {type === "bonus" && (
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className={labelCls}>Bonus Mode</label>
              ...
              <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Eligibility Rules <span className="text-gray-300 dark:text-gray-600 font-normal normal-case">(optional)</span></p>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className={labelCls}>Min Present Days<Opt /></label>
                  <input type="number" min="0" className={inputCls} value={form.min_present_days ?? ""} onChange={setF("min_present_days")} />
                </div>
                <div>
                  <label className={labelCls}>Min Payable Days<Opt /></label>
                  <input type="number" min="0" className={inputCls} value={form.min_payable_days ?? ""} onChange={setF("min_payable_days")} />
                </div>
                <div>
                  <label className={labelCls}>Max Unpaid Days<Opt /></label>
                  <input type="number" min="0" className={inputCls} value={form.max_unpaid_leave_days ?? ""} onChange={setF("max_unpaid_leave_days")} />
                </div>
              </div>
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.apply_proration_default ?? true} onChange={setF("apply_proration_default")} className="w-4 h-4 rounded accent-gray-900 dark:accent-gray-300" />
                  <span className="text-sm text-gray-700 dark:text-gray-300">Apply proration</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.require_full_attendance ?? false} onChange={setF("require_full_attendance")} className="w-4 h-4 rounded accent-gray-900 dark:accent-gray-300" />
                  <span className="text-sm text-gray-700 dark:text-gray-300">Require full attendance</span>
                </label>
              </div>
            </>
          )}
          ...
          <div className="flex justify-end gap-2 pt-2 pb-1">
            ...
            <button
              ...
              disabled={loading}
              className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-gray-900 dark:bg-gray-100 hover:bg-gray-700 dark:hover:bg-gray-300 text-white dark:text-gray-900 text-xs font-semibold transition-colors disabled:opacity-50"
            >
              {loading
