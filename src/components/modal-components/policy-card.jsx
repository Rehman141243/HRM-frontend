...
import { fmtPKR } from "./modalcomponents";
...
export default function PolicyCard({ policy, type, onEdit, onDelete, canManage }) {
  ...
  const Row = ({ label, value }) => (
    <div className="flex justify-between text-xs">
      ...
      <span className="text-gray-700 dark:text-gray-200 font-medium">{String(value)}</span>
    </div>
  );
  ...
  const renderDetails = () => {
    if (type === "attendance") return (
      <div className="space-y-1">
        ...
        ].map(([l, v]) => v != null && v !== "" && <Row key={l} label={l} value={v} />)}
        {Array.isArray(policy.holiday_dates) && policy.holiday_dates.length > 0 && (
          <div className="text-xs text-gray-400 dark:text-gray-500">
            Holidays: {policy.holiday_dates.slice(0, 3).join(", ")}{policy.holiday_dates.length > 3 ? ` +${policy.holiday_dates.length - 3} more` : ""}
          </div>
        )}
      </div>
    );
    ...
    if (type === "tax") return (
      <div className="space-y-1">
        {[
          ...
          ["Apply Proration", policy.apply_proration_default ? "Yes" : "No"],
        ].filter(([, v]) => v != null).map(([l, v]) => <Row key={l} label={l} value={v} />)}
        {Array.isArray(policy.tax_slabs) && policy.tax_slabs.length > 0 && (
          <div className="mt-2">
            <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Tax Slabs</p>
            <div className="space-y-0.5">
              {policy.tax_slabs.map((slab, i) => (
                <div key={i} className="flex justify-between text-xs bg-amber-50 dark:bg-amber-900/20 rounded px-2 py-1">
                  <span className="text-gray-500 dark:text-gray-400">Up to {slab.up_to != null ? `PKR ${slab.up_to?.toLocaleString()}` : "∞"}</span>
                  <span className="font-semibold text-amber-700 dark:text-amber-400">{slab.rate}%</span>
                </div>
              ))}
            </div>
          </div>
  ...
  return (
    <div className={cn(
      "rounded-xl border-l-4 border border-gray-100 dark:border-gray-700 bg-white dark:bg-black/5 shadow-sm overflow-hidden",
      cfg.color
    )}>
      {/* Header */}
      <div className="px-4 py-3 flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:text-gray-300 dark:hover:bg-gray-700 transition-colors"
          >
      ...
      {/* Expanded Details */}
      {expanded && (
        <div className="px-4 pb-3 pt-1 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
          {renderDetails()}
        </div>
      )}
    </div>
