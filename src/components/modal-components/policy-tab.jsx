          ...
          <div className="flex rounded-xl border border-gray-200 overflow-hidden bg-gray-50 p-1 gap-0.5">
            {policyTypeConfig.map(({ key, label, icon }) => (
              <button key={key} onClick={() => setActiveType(key)}
                className={cn("inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors",
                  activeType === key ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700")}>
                {icon}{label}
                {policies[key].length > 0 && (
                  <span className="ml-0.5 text-[9px] bg-gray-200 text-gray-600 rounded-full px-1.5 py-0.5">{policies[key].length}</span>
                )}
              </button>
            ))}
          </div>
